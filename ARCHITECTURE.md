...
**LLM / генерация — два пути, которые нельзя смешивать (ФЗ-152).**
Куратор 19.09.2026: отправка данных личного кабинета (курс, факультет,
долги по оплате — даже без ФИО) во внешний LLM API нарушает закон о
персональных данных. Поэтому:

| | Прод | Прототип |
|---|---|---|
| Режим (`LLM_MODE`) | `local` (**значение по умолчанию**) | `prototype` |
| Модель | локальная на серверах вуза (Qwen/Llama), `local_llm.py` | GPT через OpenAI API (`gpt-5.4-nano`), OpenRouter как запасной |
| Данные | любые, включая профиль ЛКС | **только публичные** (страницы сайта, eval) |
| Статус | целевая архитектура | строго временный инструмент |

Выбор делает единственная функция `llm/factory.py:build_llm()`. В режиме
`local` внешние клиенты не создаются вообще — даже если ключ OpenAI
остался в окружении после прототипа (проверено тестом
`test_local_mode_never_builds_external_client_even_with_keys_in_env`).
Переключение на прототип — только явным `LLM_MODE=prototype`, в `.env`
разработчика; на проде этой строки быть не должно.
```
infrastructure/llm/
  factory.py                    # build_llm(): local | prototype — единственная точка выбора
  ...
  local_llm.py                  # ПРОД: LLMPort к локальной модели (Ollama/vLLM, OpenAI-совместимый API)
  openai_llm.py                 # ПРОТОТИП: LLMPort через OpenAI
  openrouter_llm.py             # ПРОТОТИП: LLMPort через OpenRouter (запасной)
```
Локальная модель конкретно не выбрана (куратор: «например, Qwen/Llama») —
`LOCAL_LLM_MODEL=qwen3:8b` в `.env.example` это плейсхолдер. Реальную
генерацию на нём ещё не проверяли: нужен поднятый локальный сервер.
